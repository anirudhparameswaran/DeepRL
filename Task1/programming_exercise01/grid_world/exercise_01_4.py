...
for run in tqdm(range(n_runs), desc="Running MC experiments"):
    ...
    _, dist_ev_mc = ev_monte_carlo(env, pi, gam_0_9, s0=s0, n_samples=10000, T=128, V_true = V_pi)
    all_dist_ev_mc.append(dist_ev_mc)

    _, dist_fv_mc = fv_monte_carlo(env, pi, gam_0_9, s0=s0, n_samples=10000, T=128, V_true = V_pi)
